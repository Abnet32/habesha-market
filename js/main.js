/* ============================================================
   Habesha Market — Main JavaScript (Water-Blue Theme)
   ============================================================ */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (signupForm) {
        ...
        if (emailInput) {
            emailInput.addEventListener('input', function () {
                ...
                if (!isValidEmail(val)) { setFieldState(this, msg, 'error', 'Please enter a valid email address.'); return; }
                emailTimer = setTimeout(() => {
                    fetch('ajax/validate_email.php?email=' + encodeURIComponent(val))
                        .then(r => r.json())
                        .then(data => {
                            if (data.taken) setFieldState(emailInput, msg, 'error', 'This email is already registered.');
                            else            setFieldState(emailInput, msg, 'success', 'Email is available.');
                        });
                }, 600);
            });
        }
        ...
        signupForm.addEventListener('submit', function (e) {
            ...
            fields.forEach(f => {
                ...
                if (el && !f.validate(el.value)) { setFieldState(el, msg, 'error', f.msg); valid = false; }
            });
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function (e) {
            ...
            if (!isValidEmail(email.value)) { setFieldState(email, emailMsg, 'error', 'Please enter a valid email.'); valid = false; }
            else                             { setFieldState(email, emailMsg, 'success', ''); }

            if (pass.value.length < 6) { setFieldState(pass, passMsg, 'error', 'Password is too short.'); valid = false; }
            else                        { setFieldState(pass, passMsg, 'success', ''); }
            ...
            fetch('ajax/cart_action.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (sub) sub.textContent = 'ETB ' + parseFloat(data.subtotal).toLocaleString('en', {minimumFractionDigits:2});
                        const tot = document.getElementById('cart-total');
                        if (tot) tot.textContent = 'ETB ' + parseFloat(data.total).toLocaleString('en', {minimumFractionDigits:2});
    ...
    function setFieldState(input, msgEl, state, msg) {
        ...
        if (msgEl) { msgEl.textContent = msg; msgEl.className = 'field-msg ' + state; }
    }
